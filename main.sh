...
if [ -z "$PUBLIC_IP" ]; then
    echo -e "${RED}Failed to fetch public IP. Using 127.0.0.1 as fallback.${NC}"
    PUBLIC_IP="127.0.0.1"
fi
...
sudo systemctl restart docker  # اگر Docker نصب باشد
sudo systemctl restart apache2  # اگر Apache نصب باشد
sudo systemctl restart nginx  # اگر Nginx نصب باشد
sudo reboot
